    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Update Departments {{ $depart->name }}</h1>
                <div class="section-header-breadcrumb">
                    {{-- <div class="breadcrumb-item active"><a href="{{ route('dashboard') }}">Dashboard</a></div> --}}
                    <div class="breadcrumb-item"><a href="{{ route('pages.Depart') }}">Departments</a></div>
                    <div class="breadcrumb-item">Update Departments {{ $department->name }}</div>
                </div>
            </div>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header pb-0 px-3">
                                    <h6 class="mb-0">{{ __('Update Department') }} {{ $depart->name }}</h6>
                                </div>
                                <div class="card-body pt-4 p-3">
                                    ...
                                    {{-- <form id="depart-edit" action="{{ route('Depart.update', $hashedId) }}" method="POST"> --}}
                                        @csrf
                                        @method('PUT')
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div>
                                                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                                            name="name" value="{{ old('name', $depart->name) }}"
                                                            placeholder="exmple Food" required>
                                                        @error('name')
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="code" class="form-control-label">
                                                        <i class="fas fa-id-card"></i> {{ __('Departments Code') }}
                                                    </label>
                                                    <input type="number" class="form-control @error('code') is-invalid @enderror" id="code"
                                                            name="code" value="{{ old('code', $depart->code) }}"
                                                            placeholder="exmple Food" required>
                                                        @error('code')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                        ...
                                        <div class="d-flex justify-content-end mt-4">
                                            <a href="{{ route('pages.Depart') }}" class="btn btn-secondary">
